export class AudioStreamer {
  ...
  addPCM16(chunk: Uint8Array): void {
    ...
    console.log('Created audio buffer, duration:', audioBuffer.duration, 'seconds');
    ...
    if (!this.isPlaying && !this.currentSource && this.audioQueue.length > 0) {
      ...
      console.log('🎵 Already playing or current source exists, queued for later');
    }
  }
  ...
  private playNextBuffer(): void {
    ...
    try {
      ...
      source.onended = () => {
        ...
        if (this.audioQueue.length > 0) {
          console.log('🎵 More buffers in queue (' + this.audioQueue.length + '), continuing...');
  ...
  async resume(): Promise<void> {
    ...
    if (this.audioQueue.length > 0 && !this.isPlaying) {
      console.log('Resuming playback with', this.audioQueue.length, 'buffers in queue');
      this.isPlaying = true;
      this.playNextBuffer();
    }
  }
